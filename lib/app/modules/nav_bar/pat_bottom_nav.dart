...
class PatiantScaffoldWithBottomNavBar extends StatefulWidget {
  const PatiantScaffoldWithBottomNavBar(
  ...
  @override
  State<PatiantScaffoldWithBottomNavBar> createState() =>
      _ScaffoldWithBottomNavBarState();
}

class _ScaffoldWithBottomNavBarState
    extends State<PatiantScaffoldWithBottomNavBar> {
  int _calculateCurrentIndex(context) {
    final String location = GoRouterState.of(context).matchedLocation;
    if (location.startsWith('/${MyNamedRoutes.patientHomeScreen}')) {
      return 0;
    } else if (location.startsWith('/${MyNamedRoutes.patientCalendarScreen}')) {
      return 1;
    } else if (location.startsWith('/${MyNamedRoutes.mdPatientPage}')) {
      return 2;
    }
    return 0;
  }
  ...
  void onItemTapped(int index, BuildContext context) {
    switch (index) {
      case 0:
        GoRouter.of(context).go('/${MyNamedRoutes.patientHomeScreen}');
        break;
      case 1:
        GoRouter.of(context).go('/${MyNamedRoutes.patientCalendarScreen}');
        break;
      case 2:
        GoRouter.of(context).go('/${MyNamedRoutes.mdPatientPage}');
        break;
    }
  }
